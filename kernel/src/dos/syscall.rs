...
use super::{execution, devices, memory, system, io};
...
pub fn dos_api(regs: &mut DosApiRegisters, segments: &mut VM86Frame, stack_frame: &StackFrame) {
    ...
    match regs.ah() {
        ...
        0x40 => { // Write to file using handle
            match io::write_file(regs.bx(), regs.cx(), segments.ds as u16, regs.dx()) {
                Ok(bytes) => {
                    regs.set_ax(bytes);
        ...
        0x4a => { // Modify allocated memory blocks
            let mcb_segment = segments.es as u16;
            let paragraphs_requested = regs.bx();
        ...
        0x63 => { // Get lead byte table, not supported
            regs.set_al(0xff);
        },
